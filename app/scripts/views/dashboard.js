...
define([
...
], function(_, Backbone, Handlebars, Highcharts, highchartsMore,
  ProjectsCollection, aboutModalTpl, detailsTpls) {
  ...
  (function() {
    ...
    if (typeof Backbone === 'undefined' || Backbone === null) {
      throw new Error(
        'Backbone is not defined. Please include the latest version from http://documentcloud.github.com/backbone/backbone.js'
      );
    }
    ...
    Backbone.Modal = (function(_super) {
      ...
      Modal.prototype.render = function(options) {
        ...
        this.modalEl = Backbone.$('<div />').addClass("" + this.prefix +
          '-modal');
        if (this.template) {
        ...
        if (this.viewContainer) {
          ...
          this.viewContainerEl.addClass('' + this.prefix +
            '-modal__views');
        } else {
      ...
      Modal.prototype.getOption = function(option) {
        ...
        if (this.options && __indexOf.call(this.options, option) >= 0 &&
      ...
      Modal.prototype.delegateModalEvents = function() {
        ...
        for (key in this.views) {
          if (key !== 'length') {
            ...
            trigger = match[1];
            selector = match[2];
            _results.push(this.$el.on(trigger, selector, this.views[key],
              this.triggerView));
          } else {
      ...
      Modal.prototype.undelegateModalEvents = function() {
        ...
        for (key in this.views) {
          if (key !== 'length') {
            ...
            trigger = match[1];
            selector = match[2];
            _results.push(this.$el.off(trigger, selector, this.views[key],
              this.triggerView));
          } else {
            _results.push(void 0);
          }
      ...
      Modal.prototype.clickOutside = function(e) {
        if (Backbone.$(e.target).hasClass('' + this.prefix + '-wrapper') &&
          this.active) {
      ...
      Modal.prototype.animateToView = function(view) {
        ...
        if (previousHeight === newHeight) {
          ...
          return (_ref = this.previousView) != null ? typeof _ref.close ===
            "function" ? _ref.close() : void 0 : void 0;
        } else {
          ...
          return this.$(this.viewContainerEl).animate({
            ...
            return (_ref1 = _this.previousView) != null ? typeof _ref1.close ===
              "function" ? _ref1.close() : void 0 : void 0;
          });
        }
      };
  ...
  // Modals
  var AboutUsModal = Backbone.Modal.extend({
  ...
  var DashboardView = Backbone.View.extend({
    ...
    _onClickAboutUs: function(event) {
      var aboutUsView = new AboutUsModal();
      $('body').append(aboutUsView.render().el);
    },

    _setProjectSelect: function(argument) {
      var self = this;
      _.each(this.projects.toJSON(), function(project) {
        $('<option></option>').attr('value', project.cartodb_id).text(
          project.name_project).appendTo(self.$selectProject);
      });
    },

    _selectProject: function(event) {
      var selectedId = this.$selectProject.val(),
        project = this.projects.where({
          cartodb_id: Number(selectedId)
        })[0];
    ...
    _getHighchartSerie: function(project) {
      return [{
        ...
        data: [Number(project.get('re')), Number(project.get('bc')),
          Number(project.get('ge')), Number(project.get('ga')), Number(
            project.get('bi'))
        ],
        pointPlacement: 'on'
      }];
